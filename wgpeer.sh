...
then
	...
	ip="10.200.10."$(expr $(cat last-ip.txt | tr "." " " | awk '{print $4}') + 1)
	FQDN=$(hostname -f)
  SERVER_PUB_KEY=$(cat /etc/wireguard/publickey)
  cat wg0-client.example.conf | sed -e 's/:CLIENT_IP:/'"$ip"'/' | sed -e 's|:CLIENT_KEY:|'"$key"'|' | sed -e 's|:SERVER_PUB_KEY:|'"$SERVER_PUB_KEY"'|' | sed -e 's|:SERVER_IP:|'"$(wget -O - -q icanhazip.com)"'|' > clients/$1/wg0.conf
	echo $ip > last-ip.txt
